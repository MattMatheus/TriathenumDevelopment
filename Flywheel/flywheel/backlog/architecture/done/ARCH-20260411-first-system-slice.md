...
- shared retrieval and grounding logic should live in importable modules under `Source/` so the application API and test suite can use one code path
- `Tools/` should contain operator-facing wrappers, diagnostics, indexing helpers, and local utility entrypoints that call into the shared modules rather than owning the primary business logic
...
The first engineering wave should be split into four bounded stories:

- `STORY-20260411-app-shell-and-interaction-frame`: scaffold the web app shell, calm interaction frame, and initial actor/decision input surface
- `STORY-20260411-typed-api-contracts`: define typed request/response contracts and the thin application API boundary for the first workflow
- `STORY-20260411-grounding-bundle-retrieval`: implement actor resolution, note retrieval, and deterministic grounding bundle assembly
- `STORY-20260411-actor-reaction-workflow`: connect the UI and API to produce a grounded reaction response with visible citations and clear fact/inference boundaries

Implementation ordering:

1. typed API contracts
2. grounding bundle retrieval
3. app shell and interaction frame
4. actor reaction workflow integration

This order keeps the shared contracts and retrieval behavior stable before the end-to-end UI integration work begins.

## Architecture Handoff

- `Architecture decision`: build the first slice as a local-first web app with a thin application API over deterministic retrieval/grounding services and isolated vault access adapters
- `Alternatives considered`: direct UI-to-vault coupling, structured database as primary store, and autonomous simulation-first were all rejected for weaker grounding, poorer inspectability, or premature complexity
- `Key risks`: note heterogeneity, weak grounding being mistaken for weak model behavior, early UX overload, and accidental over-engineering
- `Follow-on implementation paths`: four engineering intake stories created for contracts, retrieval, UI shell, and end-to-end actor reaction flow
- `Next state recommendation`: review this architecture in QA, then promote the engineering stories through PM into execution order
...
Review this story in architecture QA and, if accepted, route the new engineering intake stories through PM for ordering and promotion.
