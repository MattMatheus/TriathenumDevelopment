...
- if `integrations.artifact_workflow.enabled` is `true`, the local harness `tools/artifact_workflow.sh planning --format json`
...
4. Create engineering or architecture intake artifacts in the configured backlog intake lanes using the configured templates.
...
8. If the artifact workflow integration is enabled, review the stage entry and exit commands from the local harness `tools/artifact_workflow.sh planning --format json` output and use them when they improve artifact selection or handoff durability.
   Example:
...
- do not place planning notes in backlog lanes
